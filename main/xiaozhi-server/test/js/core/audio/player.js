...
export class AudioPlayer {
    ...
    getAudioContext() {
        if (!this.audioContext) {
            ...
            log('Creating audio context, sample rate: ' + this.SAMPLE_RATE + 'Hz', 'debug');
        }
        ...
        try {
            if (typeof window.ModuleInstance === 'undefined') {
                if (typeof Module !== 'undefined') {
                    ...
                    throw new Error('Opus library not loaded, ModuleInstance and Module both missing');
                }
            }
    ...
    async startAudioBuffering() {
        ...
        this.initOpusDecoder().catch(error => {
            log(`Opus decoder pre-init failed: ${error.message}`, 'warning');
        });
        ...
        while (true) {
            const packets = await this.queue.dequeue(
                ...
                timeout,
                (count) => {
                    log(`Buffer timeout, current packet count: ${count}, starting playback`, 'info');
                }
            );
            if (packets.length) {
    ...
    enqueueAudioData(opusData) {
        if (opusData.length > 0) {
            ...
            log('Received empty audio frame, may be end signal', 'warning');
    ...
    clearAllAudio() {
        ...
        // Clear receive queue (use clear() to keep object reference)
        this.queue.clear();
