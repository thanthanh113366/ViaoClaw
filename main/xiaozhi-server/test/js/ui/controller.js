...
class UIController {
    ...
    initEventListeners() {
        ...
        // Close modal on background click (disabled for specific modals)
        const modals = document.querySelectorAll('.modal');
        modals.forEach(modal => {
            modal.addEventListener('click', (e) => {
                if (e.target === modal) {
                    // settingsModal, mcpToolModal, mcpPropertyModal can only be closed via X button
                    const nonClosableModals = ['settingsModal', 'mcpToolModal', 'mcpPropertyModal'];
                    if (nonClosableModals.includes(modal.id)) {
                        return; // Prevent closing on background click
                    }
                    this.hideModal(modal.id);
                }
            });
