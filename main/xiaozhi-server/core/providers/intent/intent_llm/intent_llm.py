...
class IntentProvider(IntentProviderBase):
    ...
    def replyResult(self, text: str, original_text: str):
        try:
            llm_result = self.llm.response_no_stream(
                system_prompt=text,
                user_prompt=(
                    "Based on the information above, reply to the user in a natural, "
                    "conversational tone. Be concise and return the result directly. "
                    "The user just said: "
                )
                + original_text,
            )
    ...
    async def detect_intent(
        ...
        if home_assistant_cfg:
            ...
            devices = []
        if len(devices) > 0:
            hass_prompt = "\nSmart home device list (location, device name, entity_id). Control via Home Assistant:\n"
